...
app.get("/", async (req, res, next) => {
  try {
    return res.status(200).json({ status: 200, message: `API Version - ${process.env.API_VERSION} : Welcome to Api gateway - Test commit on 2026/04/26 - test ommit during the viva` });
  } catch (error) {
    return next(error);
  }
});
