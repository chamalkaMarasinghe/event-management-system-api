...
app.get("/", async (req, res, next) => {
  try {
    return handleResponse(res, 200, `API Version - ${currentEnvironment.API_VERSION} : Welcome to Event service: latest !`);
  } catch (error) {
    return next(error);
  }
});
...
app.use(`/api/v${currentEnvironment.API_VERSION}/event`, eventRoutes);
